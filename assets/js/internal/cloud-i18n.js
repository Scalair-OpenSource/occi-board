 ...
 */
var registerCloudI18N = function (parent) {

  /**
   * Method: getCurrentLocaleCode
   * Returns the code (2 letters) of the language of the current page.
   */
  var getCurrentLocaleCode = parent.getCurrentLocaleCode = function () {
      return $('meta[name=sugar_date_format]').attr('content') || 'fr';
  };
  ...
  var getTimeZoneName = parent.getTimeZoneName = function () {
    return jstz.determine().name(); // Determines the time zone of the browser client
  };

  var getGeoLocation = parent.getGeoLocation = function (onSuccess, onError) {
  ...
  /**
   * Method: LoadLocales
   * This method request the translated messages from the server. It also set the format of numbers and currency.
   *
   * Parameters:
   ...
   *
   * Notes:
   * For english language, it will GET the URL: /locales/en
   */
  parent.loadLocales = function (lang, callback) {
    $.ajax({
      url: '/translations/load',
      type: 'GET'
    })
    .done(function (response) {
      if (_.isObject(response)) {
        if (response.locale) {
          parent._i18n_cache = response;
        }
        else {
          console.warn('Invalid translations returned: no locale defined!');
          if (response) {
            console.error('Response from server:', response);
          }
        }
        // Set format of currency and number in accounting lib
        accounting.settings = {
          currency: {
            symbol : __('currency-symbol'),   // default currency symbol is '$'
            format: { // controls output: %s = symbol, %v = value/number (can be object: see below)
              pos: __('currency-format-pos'), // for positive values, eg. "$ 1.00" (required)
              neg: __('currency-format-neg'), // for negative values, eg. "$ (1.00)" [optional]
              zero: __('currency-format-zero') // for zero values, eg. "$  --" [optional]
            },
            decimal : __('currency-decimal'),  // decimal point separator
            thousand: __('currency-thousand'),  // thousands separator
            precision : __('currency-precision')   // decimal places
          },
          number: {
            precision : __('number-precision'),  // default precision on numbers is 0
            thousand: __('number-thousand'),
  ...
  /**
   * Method: getLang
   * This function get the language that the user choose with a toggle button from a button group.
   * Example:
   * | parent.getLang('#lang-btn-group');
   * |
   * | <div class="control-group">
   * |     <label class="control-label" for="">Languages</label>
   * |     <div class="controls">
   * |         <span id="lang-btn-group" class="btn-group" data-toggle="buttons-radio">
   * |             <button name="lang-en" lang="en" type="button" class="btn" rel="tooltip" title="english">
   * |                 <img src="images/flags/gb.png">
   * |             </button>
   * |             <button name="lang-en" lang="fr_FR" type="button" class="btn" rel="tooltip" title="French">
   ...
   * | </div>
   */
  parent.getLang = function (btngroup) {
    $(btngroup + ' > button').each(function (index) {
      if ($(this).hasClass('active') && $(this).attr('lang')) {
   ...
   * @param  {String} id   The identifier of the message to get translation for.
   * Usually it's the message from source language.
   * @param  {Object} data [optional] Values for all variables in message.
   * @return {String}      The translation with values replacing variables.
   * If the translation does not exists a javascript console error is issued.
   *
   * @example:
   * __("Welcome!") --> "Bonjour !"
  ...
  /*
   * Global alias - Yes it's bad but so useful!
   */
  window.__ = __.bind(parent);
